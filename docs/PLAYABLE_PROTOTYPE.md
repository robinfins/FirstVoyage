...
The deck railing stays in front of the crew. Its balusters stop at world y 397 while the deck runs to 407, and that bare strip used to leave Luffy's sandals visible below the railing, detached from the legs behind it. The bottom of the deck grass is now redrawn in front as well, closing the strip so he stands in the lawn with the railing crossing his shins. It reuses the ship layer's own draw call under a clip, so the foreground copy lands pixel-identical on the background one. Ship art, local props and Luffy share a gentle visual bob. Gameplay uses stable ship-local coordinates. Ocean animation remains independent and exclusive to the Sunny. The water now uses the camera factors from the chapter layer contract — 0.22 for the sea behind the ship, 1.08 for the foreground surf — instead of being pinned to the screen, so the waves hold their place in the world and the ship sails past them. The finite wave strip is mirror-tiled to cover the wider travel; alternate copies flip, so a join is continuous regardless of how the source edges meet. Each layer is drawn at one uniform scale in world units per source pixel, taken with the cell and waterline pivot from the registration rather than hard-coded: 0.4167 for the foreground surf, 0.28 for the more distant sea behind the ship. Both are under 1/1.35, so after the world zoom the water is drawn at 0.56 screen pixels per source pixel — a downscale. It used to be blown up 2.11× across and 1.81× down, stretched as well as magnified, and from a strip that `clean_environment.py` had itself saved at a quarter of the resolution it was cleaned at. The tile is 640 world units wide now rather than 1200, so a full wave cycle fits on screen instead of 0.6 of one, and the mirror-tiling loop simply draws more copies to fill the view. `tools/verify_scene.cjs` fails if either scale stops being a single number, if the distant sea is drawn larger than the near one, or if the water is ever magnified on screen again. The lower hull is masked by foreground waves; the deck stays clear.
