...
Zoro uses 16 generated key poses in `assets/characters/zoro/zoro-atlas.png`, with two refined crossed-sword Oni Giri poses in `assets/characters/zoro/oni-giri.png`, decoded from a magenta color key once at loading. `play/zoro-art.js` supplies measured source rectangles, feet registration, and combat effects. The original image remains unmodified. His HUD crest is not from that atlas: `assets/chapter-01/ui/build_hud.py` draws `hud-zoro-crest.svg` -- three hoops with long hanging gold drops set on a diagonal, matching the reference art rather than plain rings in a row -- on the console's own medallion, so the icon matches the rest of the HUD rather than the character art. See the asset folder's `PROMPTS.md` for generation details. Animation remains key-pose based, like the existing prototype.
